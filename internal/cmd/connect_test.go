...
func TestContainerIsRunning(t *testing.T) {
	...
	for state, want := range cases {
		if got := containerIsRunning(state); got != want {
			t.Errorf("containerIsRunning(%q) = %v, want %v", state, got, want)
		}
	}
}
...
// TestBuildConnectTarget_PrefersSSHHost — the daemon-owned ssh_host wins
// over the IP, and username supplies the SSH user (the #452 contract:
// username@ssh_host).
func TestBuildConnectTarget_PrefersSSHHost(t *testing.T) {
	c := &connectContainer{Username: "cld-a7b3c1d2", SshHost: "region-a.example.com"}
	c.Network.IpAddress = "10.0.0.5"
	got, err := buildConnectTarget(c, "", "", 22)
	if err != nil {
...
// TestBuildConnectTarget_FallsBackToIP — direct mode: no ssh_host, so the
// container IP is the host. This is the pre-#452 / self-host path.
func TestBuildConnectTarget_FallsBackToIP(t *testing.T) {
	c := &connectContainer{Username: "alice"}
	c.Network.IpAddress = "10.0.0.5"
	got, err := buildConnectTarget(c, "", "", 22)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
...
// TestBuildConnectTarget_Overrides — explicit --user / --host win over
// everything the daemon reported.
func TestBuildConnectTarget_Overrides(t *testing.T) {
	c := &connectContainer{Username: "cld-x", SshHost: "region-a.example.com"}
	c.Network.IpAddress = "10.0.0.5"
	got, err := buildConnectTarget(c, "root", "bastion.example.org", 2222)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
// TestBuildConnectTarget_NoHostNoIP — a box still being placed has neither
// ssh_host nor IP; surface an actionable error rather than a broken target.
func TestBuildConnectTarget_NoHostNoIP(t *testing.T) {
	c := &connectContainer{Username: "alice"}
	if _, err := buildConnectTarget(c, "", "", 22); err == nil {
		t.Fatal("expected error when no ssh_host and no IP, got nil")
	}
}

func TestBuildConnectTarget_NoUser(t *testing.T) {
	c := &connectContainer{SshHost: "region-a.example.com"}
	if _, err := buildConnectTarget(c, "", "", 22); err == nil {
		t.Fatal("expected error when no username and no --user, got nil")
	}
}
...
func TestBuildSSHArgs(t *testing.T) {
	t.Run("interactive default port", func(t *testing.T) {
		args := buildSSHArgs(connectTarget{User: "cld-x", Host: "h.example.com", Port: 22}, "/k/id", "")
		joined := sliceJoin(args)
		...
		if containsSub(joined, "-p ") {
			t.Errorf("default port 22 should not emit -p: %q", joined)
		}
	})
	t.Run("exec + custom port", func(t *testing.T) {
		args := buildSSHArgs(connectTarget{User: "alice", Host: "10.0.0.5", Port: 2222}, "/k/id", "make build")
		joined := sliceJoin(args)
